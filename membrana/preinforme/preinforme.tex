...
%TODO label duplicado. revisar!
%TODO poner formato al texto
%TODO terminar de escribir introducción.
%TODO terminar de escribir conclusiones.
...
%Se observa también en ??? que si el radio de la célula y el campo eléctrico son muy pequeños, el potencial transmembrana no alcanza nunca 
...
En las figuras ?? se observa el voltaje transmembrana en función del ángulo tita, en diferentes instantes. 
Acá falta chamuyar algo.

%TODO mencionar comparación con fórmula del coseno?
%TODO mencionar aumento lineal del itv respecto del radio
...
\newcommand{\histoQuad}[3]{
	\newpage
	\imagenDobleTop
		{hist-radios-5e-6-#1-#2-#3KVm}{Distribución de radios en los poros para E=#3KV/m y $\alpha$=#1$\mu$m en t=\num{5e-6}s}{histo-#1-#2-#3-30e-6}
		{hist-radios-100e-6-#1-#2-#3KVm}{Distribución de radios en los poros para E=#3KV/m y $\alpha$=#1$\mu$m en t=\num{100e-6}s}{histo-#1-#2-#3-1e-3}
	\imagenDobleBottom
		{hist-radios-1e-3-#1-#2-#3KVm}{Distribución de radios en los poros para E=#3KV/m y $\alpha$=#1$\mu$m en t=\num{1e-3}s}{histo-#1-#2-#3-8e-3}
		{hist-radios-5e-3-#1-#2-#3KVm}{Distribución de radios en los poros para E=#3KV/m y $\alpha$=#1$\mu$m en t=\num{5e-3}s}{histo-#1-#2-#3-19e-3}
}
...
En las figuras ?? fueron realizados histogramas con la distribución de los radios de los poros generados en distintos instantes y para distintos campos eléctricos aplicados y distintos radios. 
faltaría poner algo más

%TODO MENCIONAR QUE LOS POROS SE CIERRAN, ENTONCES TRASNP NO SIRVE? CONCLUS?
...
Se graficaron las concentraciones de las especies de interés en escala logarítmica para curvas de nivel r = 0, con los valores iniciales en azul y finales (luego del pulso de 20ms) en rojo, ambos en escala logarítmica. Los valores de radio centrales (donde las concentraciones iniciales son menores al resto) corresponden al interior de la célula.

H+

Se observa en las figuras ??? que el catión hidrógeno ingresa muy fácilmente a la célula, obteniendo concentraciones muy altas, con varios órdenes de magnitud mayores a las iniciales.
Se observa también que el potencial aplicado incide considerablemente en la densidad de H+ en el interior de la célula. 

OH-

Se observa que se logra también el ingreso del grandes cantidades del anión hidróxido a la célula, en particular en las regiones depolarizadas, pero se requiere una campo eléctrico más grande. Por ejemplo, para 25-64 se requieren 120KV/m para lograr el ingreso en concentraciones significativas, mientras que para el anión hidrógeno bastaban 80KVm.

Na+

Se nota que el ingreso del catión sodio a la célula es mucho menor a los anteriores, requiriendo de potenciales mayores, al menos para un solo pulso de 20ms. Para células de 25 o 50 um de radio se requiere un campo eléctrico de 200Kvm para lograr un ingreso significativo al interior, obteniendo incluso en ese caso concentraciones menores a las iniciales en las regiones hiperpolarizadas. 

Cl-

Para células de 25um o 50um el anión cloruro ingresa en la célula con campos eléctricos de 160Kvm  200kvm, con concentraciones mayores en el segundo caso y concentraciones mayores a las iniciales en toda la célula. Potenciales menores no producen el ingreso en cantidades significativas, e incluso reducen las concentraciones en algunas regiones.
...
%TODO más conclusiones 
FALTA PONER ALGO MÁS!!!!\\

Para el potencial transmembrana al comienzo del puso se combinan los potenciales obtenidos por la ecuación de potencial eléctrico (\ref{eq:poisson}) con la fórmula \ref{eq:capacit} de carga de un capacitor. Esto significa que los potenciales obtenidos por el método de elementos finitos, usado para el transporte de especies, difieren de los potenciales usados para la creación y crecimiento de poros, usando dos sistemas de ecuaciones dos potenciales diferentes para un mismo instante, pudiendo afectar la precisión de los resultados. Sin embargo el efecto de la capacitancia de la membrana celular solo afecta durante los primeros microsegundos del pulso, como puede observarse en las imágenes de TMV en función del tiempo, en los cuáles el transporte de especies es casi nulo. Dado que la capacitancia de la célula no es muy grande, el valor de $(1 - e^{(-t/\tau)})$ en la fórmula \ref{eq:capacit} se hace rápidamente muy cercano a 1, y el efecto de la capacitancia se vuelve despreciable, eliminándose la discrepancia entre los potenciales usados para resolver la evolución de los poros y el transporte de especies. A pesar de no parecer influir mucho en el resultado la discrepancia, queda pendiente para el futuro obtener los resultados de potencial en todo el dominio modelando un sistema que tenga en cuenta la capacitancia de la membrana directamente en vez de usar la ecuación \ref{eq:poisson}.\\
