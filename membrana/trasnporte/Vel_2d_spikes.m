function Vel_2d_spikes(ndimy,ndimx,dir,ntop,value);
...
%    PHI = fscanf(fid4,'%f\n',[ndimy,ndimx]);
    ...
    for kk=1:ndimx
        for jj=1:ndimy
            if(PHI(kk,jj)>phimax*0.8 | PHI(kk,jj)<phimin*0.8)
                PHI2(kk,jj)=PHI(kk,jj);
            else
                PHI2(kk,jj)=0.0;
...
subplot(2,2,3);contour(Wor,30);xlabel('W')  
%contour(PHI2,v);xlabel('PHI') 
